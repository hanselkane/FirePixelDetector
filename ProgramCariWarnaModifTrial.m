...
Ihsv  = rgb2hsv(Irgb);
mareas = zeros(100); %matrix menyimpan data luasan kotak2 yg timbul
mposx = zeros(100);
mposy = zeros(100);
low = 0.001;      
high = 0.158;
...
stat = regionprops(Ilabel); %mengambil properties dari Ilabel (properties posisi(centroid), luas(area), kotak(boundingbox))
imshow(Irgb); hold on;
for x = 1 : length(stat)
    ...
    value = Ihsv(yc,xc,3);
    areas = stat(x).Area;
    txt = sprintf('Areas : %.4f\nsaturation : %.4f\nvalue : %.4f',areas,saturation,value);
